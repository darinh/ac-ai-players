...
| P-8 | Pilot-01 on AC canonical tutorial | **Working (2026-06)** — exits the Academy (via the Exit Token) into the Holtburg open world, turns in quests, buys/completes Contract Broker contracts, fights open-world mobs, and self-levels to ~L7. Slices d39965b (intent-push dedup), 7c93ce1 (beaten-kind veto), 6187654 (portal recall), c56c5f2 (directive-over-grind). 2026-06-22: cp070/72 (unlatched break-contact egress for a proven single-NPC Talk fixation — fires through monster-in-view/directive-grace, closing an Academy livelock), cp071 (tapped-out armed bot attacks visible monsters even mid-directive — engages the training golems), cp073 (act on an NPC's dialogue give-request for a held item instead of Talk-looping). Academy progression past the Talk steps + combat engagement + death/lifestone recovery confirmed live; reliability remains LLM-variance-sensitive run-to-run. |
