...
internal static class InventoryFailurePolicy
{
    ...
    /// in an explicit Wield/Dequip transaction, or an item explicitly picked up.
    /// </summary>
    internal static bool ShouldSurfaceInventoryFailure(
        uint errorType,
        uint itemGuid,
        uint? pendingGiveItemGuid,
        IReadOnlySet<uint> inventoryTransactionGuids,
        IReadOnlySet<uint> pickupDispatchedGuids)
        => errorType != 0
           || (pendingGiveItemGuid is uint pg && pg == itemGuid)
           || inventoryTransactionGuids.Contains(itemGuid)
