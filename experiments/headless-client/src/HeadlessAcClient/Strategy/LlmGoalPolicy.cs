...
internal sealed class LlmGoalPolicy : IGoalPolicy
{
    ...
    private Goal? ProposeGoalCore(WorldStateProjection world, EventStream events, Goal? currentGoal)
    {
        ...
        if (_redriveIntentId is not null && _redriveGoal is not null)
        {
            ...
            var redriveApproachReached = IsApproachToReachedTarget(_redriveGoal, world);
            if (stuck || landblockChangedSinceLook || semanticRejectSinceLook
                ...
                || budgetExhausted || redriveApproachReached
                || confirmationRequested || teammateSighted)
            {
                Console.WriteLine(
                    ...
                    (stuck ? "stuck-timeout"
                     ...
                     : redriveApproachReached ? "approach-target-reached"
        ...
        var stickyApproachReached =
            IsApproachToReachedTarget(_lastLlmGoal, world);
        if (currentGoal is null && stickyApproachReached
            && !hasNonPickerExternal && !pickerArrived && !pickerStartWake)
        {
            Console.WriteLine(
                $"[strategy] sticky re-emit broken: reached named {_lastLlmGoal!.Kind} target " +
        ...
        if (currentGoal is null && stickyUntargetedExplore && _lastLlmGoal is not null)
        {
            var blockReason =
                ...
                : stickyApproachReached ? "approach-reached"
                : "other";
        ...
        DateTimeOffset nowUtc)
    {
        ...
        if (IsApproachToReachedTarget(goal, world))
        {
            Console.WriteLine(
                $"[llm-dedup] dropping LLM {goal!.Kind} target={goal.Target}" +
                " — target already reached; a navigate-only approach cannot interact; deferring to fallback.");
            _training?.RecordParseError(decisionId,
                "dropped-by-dedup: approach (Explore/GoTo) toward an already-reached target (no-op)");
            return _fallback.ProposeGoal(world, events, currentGoal);
        }
    ...
    // Goal-based reached check: true iff `goal` is a DIRECTED approach — a
    // TARGETED Explore (name= exact / name_contains= substring; not the
    // untargeted "anywhere" sentinel) OR a GoTo (always targeted) — whose OWN
    // target matches a `Visible nearby` object within reach. Both verbs are
    // navigate-only (arrival sends no opcode), so once the bot is already at the
    // target, re-driving the same approach makes no progress; the Motor uses this
    // to force a fresh LLM decision AT arrival (so the LLM can pick the follow-up
    // interaction verb) instead of re-driving in place. Keys on the SPECIFIC goal
    // being evaluated, so a NEW approach toward a DIFFERENT (not-yet-reached)
    // target is NOT matched. Pure motor-geometry over the bot's own goal + visible
    // objects; no game knowledge, no source-side interaction decision.
    internal static bool IsApproachToReachedTarget(Goal? goal, WorldStateProjection world)
    {
        if (goal is null) return false;
        var isApproach = (goal.Kind == GoalKind.Explore && !IsUntargetedExploreGoal(goal))
                         || goal.Kind == GoalKind.GoTo;
        if (!isApproach) return false;
        if (world?.Visible is null) return false;
        var name = goal.Target?.Name;
        var nameContains = goal.Target?.NameContains;
        var guid = goal.Target?.Guid;
        if (string.IsNullOrWhiteSpace(name) && string.IsNullOrWhiteSpace(nameContains) && guid is null)
            return false;
        // guid= is AUTHORITATIVE when present: the approach executor resolves a
        // GUID-addressed target FIRST (HandshakeDriver prefers goal.Target.Guid) and
        // a guid uniquely identifies ONE object, so "reached" means THAT exact object
        // is visible within reach — and NOT that some DIFFERENT nearby object happens
        // to match a name also on the selector. Mirror the executor's guid-first
        // semantics: when a guid is given, decide solely on it and do not fall through
        // to the name paths (avoids wrongly breaking a still-active directed approach).
        if (guid is uint tg)
        {
            foreach (var v in world.Visible)
                if (v.Guid == tg)
                    return v.Distance is float gd && gd <= ReachedExploreTargetDistanceUnits;
            return false;
        }
    ...
    // Sibling: repeated-Pickup-of-a-fuzzy-DESCRIPTOR window + threshold. A Pickup carrying a
    // type-descriptor selector (short_desc~= / name_contains) rather than a concrete name
    // expresses "pick up any item matching this description"; when nothing in view matches it
    // resolves to MISS. The name-keyed loop machinery parses only name="...", so it cannot see
    ...
    // IsApproachToReachedTarget only covers a target that IS a visible reached object;
        ...
        WorldStateProjection world, EventStream events, DateTimeOffset since)
    {
        ...
        foreach (var ge in events.RecentGoalEmissions().Where(e => !string.IsNullOrEmpty(e.Text)))
        {
            ...
            // in the item field is never read as the Pickup target.
    ...
    {
        var sd = System.Text.RegularExpressions.Regex.Match(segment, "short_desc~=\"([^\"]+)\"");
...
""");
        ...
        // empty corpses while a large unspent-XP balance piles up. Surface the escape: raising
        // stats with that XP (survive + hit harder) is itself how it becomes able to beat the
        // tougher, loot-bearing foes. Gated on HasNoUsableWeaponAnywhere (no wieldable/bagged
        // weapon — so it never competes with a direct Wield of an in-bag weapon) AND meaningful
        // unspent XP (re-checked like every other unspent-XP surface); the wording also
        // subordinates to any nearby/affordable-vendor weapon. Generic levers + a general loot
        // rule; the LLM decides.
        if (HasNoUsableWeaponAnywhere(world)
            && world.Self.AvailableExperience is long armUnspent
            && ShouldSurfaceUnspentXp(armUnspent, MinMeaningfulUnspentXp))
            sb.AppendLine($"- ARM BY GETTING STRONGER: FIRST arm with any weapon available to `Wield`/`Pickup`/`Buy` (per SELF-ARM above) — that is faster than raising stats. But if NO weapon is available to you and even the weakest monsters you CAN beat drop nothing useful while you sit on unspent XP ({armUnspent}), do NOT hoard it — raise endurance (more max HP so you survive longer) and your damage stat (strength drives fist damage, and with coordination fist accuracy) so you can survive and beat the TOUGHER monsters that DO drop weapons and coin. Spending XP to become able to kill better, loot-bearing foes is DIRECTED progress toward arming, not a distraction from it.");
        ...
        if (aPlayerIsInView || inFellowshipForChat || canTellMonarch)
        {
            ...
            if (aPlayerIsInView)
                sb.AppendLine(
                    "- A `player` is in view. You MAY `Say` a line ALOUD (words in `message`, no `channel`); " +
                    "nearby players and creatures hear it as local chat. OPTIONAL — you decide whether and what.");
            if (inFellowshipForChat)
                sb.AppendLine(
        ...
        // The bot re-emits `Pickup` carrying a fuzzy type-DESCRIPTOR (short_desc~= / name_contains)
    ...
    internal static bool TryParseGoal(string json, out Goal? goal, out string? error)
    {
        ...
        try
        {
            ...
            {
                error = "Say goal requires a non-empty message";
                return false;
            }
            goal = parsed;
            return true;
