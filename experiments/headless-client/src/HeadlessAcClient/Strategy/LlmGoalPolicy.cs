...
{
  "goal_id": "<new uuid>",
  "kind": "Give" | "Use" | "Attack" | "Pickup" | "Wield" | "GoTo" | "Talk" | "Wait" | "Explore" | "RaiseAttribute" | "RaiseVital",
  ...
  "amount": number | null,   // RaiseAttribute/RaiseVital only: whole positive XP; target.name = the attribute/vital
  "rationale": string,
  "priority": 1..10,
...
""");
        ...
        sb.AppendLine("- SPEND XP: `## Self` shows `experience: N total, M unspent`. Unspent XP is wasted until invested — raise an attribute with `RaiseAttribute{target: {name: \"endurance\"}, amount: <positive whole XP>}` (names: strength, endurance, quickness, coordination, focus, self), or a vital pool with `RaiseVital{target: {name: \"health\"}, amount: <XP>}` (names: health, stamina, mana). A positive `amount` is REQUIRED. Endurance/health raise MAX HEALTH. Invest a chunk when max HP is low.");
