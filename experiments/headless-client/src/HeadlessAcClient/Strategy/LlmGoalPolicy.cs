        ...
        if (world.Self.AvailableExperience is long endcapUnspent && endcapUnspent > 0)
        {
            ...
            if (endcapHasSwings)
            {
                ...
                sb.AppendLine(
                    ...
                    "and NOT fixed by endurance/health (which only raise max HP); strength is the wrong lever for it too (strength's main " +
                    $"effect is DAMAGE, not accuracy). Being OUT-DAMAGED after your hits LAND points to {hurtLever}; dying " +
