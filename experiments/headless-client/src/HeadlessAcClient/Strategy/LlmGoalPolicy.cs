...
""");
        ...
        sb.AppendLine("- Combat: creatures tagged `monster` are appropriate combat targets and grant XP and loot. Creatures tagged `npc` are civilians — talk to or trade with them, do NOT attack. When a `monster` is visible nearby AND a weapon is wielded (visible in inventory as `wielded@...`) AND no server hint or item short_desc gives a more specific quest action, engaging it is appropriate — emit `Attack{target: name=\"X\"}`, but choose WHICH monster and WHEN to disengage per the COMBAT SAFETY rule below rather than blindly attacking. Combat is the primary source of XP outside of NPC quests.");
        sb.AppendLine("- LEVELING is core progress — be PROACTIVE, not just reactive. `## Self` shows your current `level`. Growing stronger by gaining combat experience is a primary objective in its own right, even when nothing has attacked you. When you are combat-ready (a weapon shows `wielded@` in `## Inventory`) AND you are NOT mid an explicit server/quest directive: if a `monster` is in view, `Attack` it (subject to the COMBAT SAFETY rule below); if NO `monster` is in view, do NOT loiter among town `npc`s once their dialog is exhausted — emit `Explore{target: {name: \"anywhere\"}}` to travel toward open areas where monsters live. Seek the experience out; do not wait to be attacked first. A low-level character that only talks to NPCs and never fights will stay weak and cannot beat the game.");
        sb.AppendLine("- COMBAT SAFETY & PACE: fight roughly one `monster` at a time — if several are clustered or more than one shows the `HOSTILE` tag, back off and engage them singly rather than pulling the whole group. The reliable danger signals available to you are your `deaths` count and, when shown, your `health` in `## Self` (monster levels are NOT given — judge difficulty from OUTCOMES, not numbers). If your `deaths` count rises, or your `health` is shown and low, DISENGAGE — emit `Explore` to break away and recover, and AVOID re-attacking the same KIND of monster that just defeated you (pick a different or more distant target instead). Do NOT grind one spot forever: explicit server/quest directives and looting fresh corpses outrank optional combat — after defeating a monster, loot its corpse (see the looting rule), then reassess whether a quest objective or a new area is the better next move.");
        ...
        sb.AppendLine("- PASSAGE-OPENED is not progress: A `door` (and any `openable` that is NOT a container/corpse/chest) is a passage — Using it opens a path but does NOT move you. Opening such a passage does NOT count as 'something concrete changed' for the loop-break rule above; the ONLY thing that counts as progress is actually MOVING to a new area (your `current cell` or landblock changing, or previously-unseen objects appearing in `Visible nearby`). After you Use a door once, do NOT Use it again while you are still in the same spot — instead emit `Explore{target: {name: \"anywhere\"}}` (or a goal targeting something beyond it) to travel THROUGH the now-open passage and reach the room on the other side. Re-Using an already-opened door just re-opens it and wastes the turn. (This does NOT apply to `chest`/`corpse` containers, which you Use to reveal loot, then `Pickup`.)");
        ...
        sb.AppendLine("- HUNT EXCURSION (leave a tapped-out safe zone to find monsters): a safe town/hub gives finite quest and dialog progress, but combat XP requires monsters, which do NOT spawn inside safe zones — you must travel OUT to the surrounding open country to find them. When you are combat-ready (a weapon shows `wielded@` in `Inventory`), NO `monster` is anywhere in `Visible nearby`, you have NO un-acted-on server/quest directive naming a specific next target (re-talking an NPC who gives no NEW dialog and browsing vendors do NOT count as directives), AND `minutes in current landblock` (see the `Location & recency` section) is more than a few minutes with local quest/dialog progress dried up (no new level, quest item, or unique server hint arriving in that time), the safe zone is TAPPED OUT for now. Emit `Explore{target: {name: \"anywhere\"}}` to head for unexplored open country where huntable monsters live. CROSSING OUT OF A SAFE ZONE TAKES MANY TICKS of walking: keep choosing `Explore{target: {name: \"anywhere\"}}` every cycle — do NOT revert to talking to the same still-visible town NPCs mid-excursion (your own `Explore` goal in recent history does NOT mean the excursion is done) — until your `landblock` (in `Self`) actually changes OR a `monster` comes into view (then `Attack` it). Genuine quest progress always interrupts the hunt: the MOMENT a NEW server/quest directive, a new quest item, danger, or a fresh dialog step appears, drop the excursion and act on that instead. Only leave once local progress has truly run dry — quest progress outranks an optional hunt.");
