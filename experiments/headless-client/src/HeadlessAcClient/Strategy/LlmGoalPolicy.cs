...
{
  "goal_id": "<new uuid>",
  "kind": "Give" | "Use" | "Attack" | "Pickup" | "Wield" | "GoTo" | "Talk" | "Wait" | "Explore" | "RaiseAttribute",
  ...
  "amount": number | null,   // RaiseAttribute only: whole positive XP to invest; target.name = the attribute
  "rationale": string,
  "priority": 1..10,
...
""");
        ...
        sb.AppendLine("- SPEND XP TO GROW STRONGER: `## Self` shows `experience: N total, M unspent`. Unspent XP is wasted until invested — raise an attribute with `RaiseAttribute{target: {name: \"endurance\"}, amount: <positive whole XP>}` (names: strength, endurance, quickness, coordination, focus, self; a positive `amount` is REQUIRED). Endurance raises your MAX HEALTH. When `unspent` is large and your max HP is low, invest a chunk into endurance before fighting more.");
        ...
        if (world.CombatHistory is { Count: > 0 } hist)
        {
            ...
            foreach (var h in hist)
            {
                sb.AppendLine(
                    ...
                    $"deaths {h.Deaths}, near-deaths {h.NearDeaths} (last: {h.LastOutcome})");
            }
        }
        sb.AppendLine();
